...
var mochaHook = function (reportManager, reportId, finishedCallback) {
	...
	var reporterIdStack = [{ type:'root', id:reportId }],
		reporterIdCounter = 0;
	...
	return function (runner) {
		...
		runner.on('suite', function (suite) {
			var id = "suite-" + (reporterIdCounter++) + '_' + reportId,
				parentIdObj = reporterIdStack[0];
		...
		runner.on('test', function (test) {
			var id = "test-" + (reporterIdCounter++) + '_' + reportId,
				parentIdObj = reporterIdStack[0];
